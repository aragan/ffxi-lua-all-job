--based on Sel-Display from Selindrile

--[[

--how to setup in your lua file

function get_sets()
	include('Xen-Display.lua')
...
function job_state_change(stateField, newValue, oldValue)
    ...
    --add that at the end of job_state_change
...
function user_setup()
	...
	--add that at the end of user_setup
    if init_job_states then init_job_states({"WeaponLock","MagicBurst","Auto_Kite"},{"IdleMode","OffenseMode","HybridMode","RangedMode","WeaponskillMode","PhysicalDefenseMode","CastingMode","TreasureMode"}) end
end
=========================================
...
function init_job_states(job_bools,job_modes)
    ...
    if weaponskill_autoaftermath_get_job_states_boolean_list then
    	stateBool = weaponskill_autoaftermath_get_job_states_boolean_list(job_bools)
    end

    if weaponskill_autoaftermath_get_job_states_option_list then
    	stateList = weaponskill_autoaftermath_get_job_states_option_list(job_modes)
    end
...
function update_job_states()
    ...
    local labels = {
		...
		SubSet = "Sub Set",
		AutoAftermathLevel = "AM Lv.",
		AutoAftermathWeaponType = "AM Type",
    }
    ...
    for i,n in pairs(stateBool) do
        ...
        if state[n].index then
			...
			elseif n == 'AutoAftermath' then
				if state.AutoAftermath and state.AutoAftermath.value == true and state.AutoAftermathLevel and state.AutoAftermathLevel.value and state.AutoAftermathWeaponType and state.AutoAftermathWeaponType.value then
					stateBox:append(string.format("%sAutoAftermath Lv."..state.AutoAftermathLevel.value.." ("..state.AutoAftermathWeaponType.value..")", clr.h, clr.n))
					stateBox:append(spc)
				end
			else
				if labels[n] then
    ...
    for i,n in ipairs(stateList) do
		...
		elseif n == 'AutoSambaMode' then
			if state.AutoSambaMode and state.AutoSambaMode.value ~= 'Off' then
				stateBox:append(string.format("%sAuto Samba: %s%s    ", clr.w, clr.h, state.AutoSambaMode.value))
			end
		elseif n == 'IdleMode' then
			if state.IdleMode and state.IdleMode.value ~= 'Normal' and state.DefenseMode and state.DefenseMode.value == 'None' then
				stateBox:append(string.format("%s%s: ${%s}    ", clr.w, labels[n], n))
			end
			if state.Kiting and state.Kiting.value then
				stateBox:append(string.format("%sKiting: %sOn    ", clr.w, clr.h))
			end
		elseif n == 'Passive' then
			if state.Passive and state.Passive.value ~= 'None' and state.DefenseMode.value == 'None' then
				stateBox:append(string.format("%s%s: ${%s}    ", clr.w, labels[n], n))
			end
		elseif n == 'TreasureMode' then
			if (state.TreasureMode and state.TreasureMode.value ~= 'None' or player.main_job == 'THF') and state.DefenseMode.value == 'None' then
		...
		elseif n == 'WeaponskillMode' then
			if state.WeaponskillMode and state.WeaponskillMode.value ~= 'Match' then
				stateBox:append(string.format("%sWeaponskill: %s%s    ", clr.w, clr.h, state.WeaponskillMode.value))
			end
			if state.SkillchainMode and state.SkillchainMode.value ~= 'Off' and state.DefenseMode.value == 'None' then
		...
		elseif n == 'RuneElement' then
			if state.AutoRuneMode and not state.AutoRuneMode.value and (player.main_job == 'RUN' or player.sub_job == 'RUN') then
		...
		elseif n == 'LearningMode' then
			if state.LearningMode and state.LearningMode.value and state.DefenseMode.value == 'None' then
				stateBox:append(string.format("%sLearning Mode: %sOn    ", clr.w, clr.h))
			end
		elseif n == 'CompensatorMode' then
			if state.CompensatorMode and state.CompensatorMode.value ~= 'Never' then
				stateBox:append(string.format("%sCompensator: %s%s    ", clr.w, clr.h, state.CompensatorMode.value))
			end
		elseif n == 'CarnMode' then
			if state.CarnMode and state.CarnMode.value ~= 'Never' then
				stateBox:append(string.format("%sCarn Mode: %s%s    ", clr.w, clr.h, state.CarnMode.value))
			end
		elseif n == 'DrainSwapWeaponMode' then
			if state.DrainSwapWeaponMode and state.DrainSwapWeaponMode.value ~= 'Never' then
				stateBox:append(string.format("%sDrain Swap: %s%s    ", clr.w, clr.h, state.DrainSwapWeaponMode.value))
			end
		elseif n == 'ExtraSongsMode' then
			if state.ExtraSongsMode and state.ExtraSongsMode.value ~= "None" then
				stateBox:append(string.format("%sSongs: %s%s    ", clr.w, clr.h, state.ExtraSongsMode.value))
			end
		elseif n == 'DanceStance' then
			if state.DanceStance and state.DanceStance.value ~= "None" then
				stateBox:append(string.format("%sDance: %s%s    ", clr.w, clr.h, state.DanceStance.value))
			end
		elseif n == 'Stance' then
			if state.Stance and state.Stance.value ~= "None" then
				stateBox:append(string.format("%sStance: %s%s    ", clr.w, clr.h, state.Stance.value))
			end
		elseif n == 'WeaponSet' then
			if state.WeaponSet and state.WeaponSet.value then
				if state.AbyWeaponSet and state.AbyWeaponSet.value and state.AbyWeaponSet.value ~= 'None' then
					stateBox:append(string.format("%s%s: %s%s", clr.w, labels[n], clr.h, state.AbyWeaponSet.value:sub(4,-1)))--aby sets starting with Aby
				else
					stateBox:append(string.format("%s%s: %s${%s}", clr.w, labels[n], clr.h, n))
					if state.SubSet and state.SubSet.value then
						stateBox:append(string.format("%s / %s%s%s", clr.n, clr.h, state.SubSet.current, clr.n))
					end
				end
				stateBox:append(spc)
			end
		elseif n == 'AutoAftermathLevel' or n == 'AutoAftermathWeaponType' then
			if state.AutoAftermath and state.AutoAftermath.value == true then
				stateBox:append(string.format("%s%s: ${%s}    ", clr.w, labels[n], n))
			end
		else
			if labels[n] then
...
windower.raw_register_event('incoming chunk', function(id, data)
    if id == 0x00A and stateBox and state.DisplayMode and state.DisplayMode.value then
        stateBox:show()
    end
end)
